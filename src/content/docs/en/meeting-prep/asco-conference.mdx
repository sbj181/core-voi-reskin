...
Throughout its history, BMS has made significant advancements in oncology, hematology, immunology and cardiovascular diseases. With multiple breakthroughs in these diseases with high unmet needs, BMS has helped transform patients’ lives thought substantial improvements in both life expectancy and quality of life. 
...
{/* use tag name or resource title below */}
    ...
    {getRelatedResources([''], ['Oncology Cross Solid-Tumor Value Messages for Access & Reimbursement Stakeholders', 'BMS Website', 'Benefits Review Guide']).map(relatedResource => (
